...
class PiholeConfigTest {
    ...
    void testHasValidAuthToken() {
        var validConfig = new PiholeConfig("192.168.1.1", 80, "http", "mytoken123");
        assertTrue(validConfig.hasValidAuthToken());

        var invalidConfig1 = new PiholeConfig("192.168.1.1", 80, "http", "");
        assertFalse(invalidConfig1.hasValidAuthToken());

        var invalidConfig2 = new PiholeConfig("192.168.1.1", 80, "http", null);
        assertFalse(invalidConfig2.hasValidAuthToken());

        var invalidConfig3 = new PiholeConfig("192.168.1.1", 80, "http", "   ");
    ...
    void testIsFullyValid() {
        ...
        var fullyValidConfig = new PiholeConfig("192.168.1.1", 80, "http", "token123");
        assertTrue(fullyValidConfig.isFullyValid());

        // Valid address, no token
        var noTokenConfig = new PiholeConfig("192.168.1.1", 80, "http", "");
        assertFalse(noTokenConfig.isFullyValid());

        // No address, valid token
        var noAddressConfig = new PiholeConfig("", 80, "http", "token123");
        assertFalse(noAddressConfig.isFullyValid());

        // Neither address nor token
        var neitherConfig = new PiholeConfig("", 80, "http", "");
